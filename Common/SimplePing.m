...
@property (nonatomic, assign, readwrite          ) uint8_t     timeToLiveProperty;
...
- (instancetype)initWithHostName:(NSString *)hostName {
    ...
    if (self != nil) {
        ...
        self->_identifier = (uint16_t) arc4random();
         self.timeToLiveProperty = (uint8_t) arc4random();
    }
    return self;
}
...
- (void)sendPingWithData:(NSData *)data {
    ...
    if (self.socket == NULL) {
        ...
        CFSocketNativeHandle socket = CFSocketGetNative(self->_socket);
        struct timeval tv;
        tv.tv_sec  = 0;
...
- (NSUInteger)icmpHeaderOffsetInIPv4Packet:(NSData *)packet {
    ...
    if (packet.length >= (sizeof(IPv4Header) + sizeof(ICMPHeader))) {
        ...
        self.timeToLiveProperty = ipPtr->timeToLive;
...
- (BOOL)validatePing4ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr {
    ...
    icmpHeaderOffset = [self icmpHeaderOffsetInIPv4Packet:packet];
